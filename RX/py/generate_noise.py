...
def apply_channel(signal, snr_db=30, taps=[1.0, 0.2*np.exp(1j*np.pi/4), 0.1*np.exp(-1j*np.pi/3)]):
    """Applies multipath convolution and Additive White Gaussian Noise."""
    # 1. Multipath Fading (Convolution with channel taps)
    # mode='same' ensures the array length stays identical for your RTL memory bounds
    faded_signal = np.convolve(signal, taps, mode='same')
...
def generate_ofdm_mem(image_path, n_fft=128, cp_len=32, dim=32, f_cfo_norm=0.0035, snr_db=30, front_noise_len=0):
    ...
    # =========================================================================
    # 2.5. PREPEND FRONT NOISE
    # =========================================================================
    if front_noise_len > 0:
        # Generate unit-variance complex Gaussian noise
        front_noise = (np.random.randn(front_noise_len) + 1j * np.random.randn(front_noise_len)) / np.sqrt(2)
        tx_signal.extend(front_noise)
    ...
    # =========================================================================
    # 4.5. CHANNEL INJECTION (MULTIPATH + AWGN)
    # =========================================================================
    tx_signal = apply_channel(tx_signal, snr_db=snr_db)
...
# Execution
tx_bits = generate_ofdm_mem("input.jpeg", dim=32, f_cfo_norm=0.00001, snr_db=20, front_noise_len=0)